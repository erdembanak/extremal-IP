"""Independent complement-variable encoding to audit the completed catalogue.
...
def audit_case(e, pattern, degrees, timeout=90):
    ...
    for subset in combinations(range(17), 8):
        model.add(sum(y[pair] for pair in combinations(subset, 2)) <= 27)
    ...
    while time.monotonic() - started < timeout:
        ...
        if (not props['triangle_free'] or props['alpha'] > 7 or props['edges'] != e
                or tuple(props['degrees']) != degrees):
            raise RuntimeError('Audit encoding produced an invalid graph')
        assignments += 1
        if props['factor_critical']:
            graph = nxgraph(edges)
            matches = [i for i, target in enumerate(expected) if nx.is_isomorphic(graph, target)]
            if len(matches) != 1:
