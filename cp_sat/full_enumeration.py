"""Run all ten Earth--Moon degree cases, with checkpoints and aggregate status."""
...
def aggregate(root):
    ...
    result = {'updated_utc': datetime.now(timezone.utc).isoformat(),
              'scope': 'n=17, triangle-free, maximum degree <=7, alpha<=7, factor-critical',
...
def run_case(root, edges, pattern, seconds, workers, round_no):
    ...
    cmd = [sys.executable, str(Path(__file__).with_name('enumerate_graphs.py')),
           '--edges', str(edges), '--alpha-max', '7', '--factor-critical',
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=Path('cp_sat/runs/full'))
