...
void solve(int d, int N){
	...
	IloIntVarArray degree = CreateIntVarArray(env, N, "d", 0, d);

	IloObjective objective = IloMaximize(env);
	model.add(objective);

	float perturbate_amount = 0.0001;
	float starting_point = 0.5 - (N/2)*perturbate_amount;

	for (int i=0; i < N; ++i){
		cout << "perturbated " << i*perturbate_amount << endl;
		objective.setLinearCoef(degree[i], 0.5 + i*perturbate_amount);
	}
	for (int i=0; i < N; ++i){
		for (int j=i; j < N; ++j){
			model.add(degree[i] >= degree[j]);
		}
	}
	/*
	for (int i = 0; i < N; ++i)
...
*/
	...
	IloRangeArray degreeConstraints = CreateRangeArray(env, N, "degree", 0, 0);

	for (int i = 0; i < N; ++i)
	{
		degreeConstraints[i].setLinearCoef(degree[i], -1);
	...
	//ofstream MyFile("results_ordering.txt", std::ios_base::app);
...
int main() {
	...
	//ofstream MyFile("results_ordering.txt");
